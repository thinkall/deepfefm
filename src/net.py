...
class FEFM(nn.Layer):
    def __init__(self, sparse_feature_number, sparse_feature_dim,
        ...
        self.dense_w_one = paddle.create_parameter(
            ...
            attr=paddle.ParamAttr(regularizer=paddle.regularizer.L2Decay(1e-6)),
            dtype='float32',
        ...
        for fi, fj in itertools.combinations(range(self.num_fields), 2):
            ...
            self.field_embeddings[field_pair_id] = paddle.create_parameter(
                ...
                attr=paddle.ParamAttr(regularizer=paddle.regularizer.L2Decay(1e-7)),
                dtype='float32',
                default_initializer=paddle.nn.initializer.TruncatedNormal(
                    mean=0.0,
                    std=self.init_value_ /
    ...
    def forward(self, sparse_inputs, dense_inputs):
        ...
        sparse_emb_one = self.embedding_one(sparse_inputs_concat)  # [batch_size, sparse_feature_number, 1]

        _dense_emb_one = paddle.multiply(dense_inputs, self.dense_w_one)  # [batch_size, dense_feature_number]
        dense_emb_one = paddle.unsqueeze(_dense_emb_one, axis=2)  # [batch_size, dense_feature_number, 1]
        ...
        fefm_interaction_embedding = paddle.concat(pairwise_inner_prods, axis=1)  # [batch_size, num_fields*(num_fields-1)/2]
...
class DNN(paddle.nn.Layer):
    def __init__(self, sparse_feature_number, sparse_feature_dim,
        ...
        self.input_size = int(dense_feature_dim + self.sparse_num_field * sparse_feature_dim
                              + self.num_field*(self.num_field-1)/2)
